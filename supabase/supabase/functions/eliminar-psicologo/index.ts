// Baja (hard delete) de cuentas de Psicólogo (Superadmin). El borrado
// desasigna automáticamente al psicólogo de todas sus instituciones
// antes de eliminarlo — no bloquea pidiendo desasignación manual previa.
//
// Orden de borrado, OBLIGATORIO por la cadena de Foreign Keys (todas
// ON DELETE NO ACTION, sin cascada — ver informe de base de datos):
...
// Invertir este orden falla: no se puede borrar `usuarios` mientras
// todavía tenga filas en `psicologo_institucion` apuntándole, ni borrar
// `auth.users` mientras `usuarios` todavía exista.
//
