// Edición de cuentas de Psicólogo (Superadmin). Actualiza nombre y/o
// correo — nunca la contraseña (eso queda para "Olvidé mi contraseña",
// que el propio psicólogo gestiona desde su cuenta).
//
// Si el correo cambia, se sincroniza PRIMERO contra `auth.users` (vía
// `admin.updateUserById`) y solo después se actualiza `usuarios`. Si el
// paso de Auth falla, no se toca la tabla — así `usuarios.email` y
// `auth.users.email` nunca quedan desincronizados (esa desincronización
// fue la causa raíz de un bug real de "correo en blanco" en una sesión
// anterior).
//
// Misma verificación de sesión + rol superadmin que crear-psicologo.ts —
// ver esa función para el detalle del patrón caller/admin.
